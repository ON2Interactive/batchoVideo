...
const ExportDialog: React.FC<Props> = ({ 
  ...
  const [format, setFormat] = useState<'png' | 'video'>(hasVideo ? 'video' : 'png');
      ...
      <div className="bg-[#121214] border border-zinc-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${
                  format === 'png' ? 'border-blue-500 bg-blue-600/5 text-white' : 'border-zinc-800 bg-zinc-900 text-zinc-500 hover:border-zinc-700'
                }`}
              >
                <ImageIcon size={24} />
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all ${
                      isSelected ? 'bg-zinc-800 border-zinc-600 text-white' : 'bg-transparent border-transparent text-zinc-500 hover:bg-zinc-900'
                    }`}
                  >
                    <div className="flex items-center gap-3">
        ...
        <div className="p-6 bg-zinc-900/50 border-t border-zinc-800">
          <button
            ...
            className={`w-full h-12 rounded-xl font-bold flex items-center justify-center gap-2 transition-all active:scale-[0.98] shadow-xl ${
              selectedRes.premium && !isPro 
